...
class TestSecureTokenManager:
    ...
    def test_token_encryption_varies(self):
        ...
        encrypted1 = manager.encrypt_token(token.encode())
        encrypted2 = manager.encrypt_token(token.encode())
        ...
        assert decrypted1.decode() == token
        assert decrypted2.decode() == token
